/**
 * socketio_direct_fix.js - Direct fixes for Socket.IO connections to localhost
 * 
 * This script directly intercepts Socket.IO before it's initialized and applies
 * critical fixes specifically to handle the localhost to localhost conversion issue.
 * 
 * IMPORTANT: This script must be loaded BEFORE socket.io.js
 */

// This script must run before Socket.IO is loaded, so we apply it at the global level
...
(function() {
    ...
    // Override document.domain with a getter/setter that always changes localhost to localhost
    try {
        Object.defineProperty(document, 'domain', {
            ...
            set: function(value) {
                if (value === 'localhost') {
                    console.log('⚠️ Intercepted attempt to set document.domain to localhost, using localhost instead');
    ...
    // Helper to check if a URL uses localhost
    ...
    window.socketioDirectFix = {
        ...
        analyzeConnectionPatterns: function() {
            ...
            var failedUrls = patterns.filter(p => p.originalUrl.includes('localhost'));
            
            if (failedUrls.length > 0) {
                console.warn('⚠️ Detected multiple connection attempts to localhost addresses');
                ...
                if (diagEl) {
                    ...
                    html += 'Detected ' + failedUrls.length + ' attempts to connect to localhost addresses.<br>';
    ...
    var checkSocketIO = setInterval(function() {
        if (window.io) {
            ...
            window.io = function() {
                ...
                // Always convert localhost to localhost
                if (typeof url === 'string' && url.includes('localhost')) {
